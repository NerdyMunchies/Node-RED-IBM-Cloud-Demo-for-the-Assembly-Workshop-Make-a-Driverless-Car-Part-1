...
One of the **dashboard** nodes missing in the **Raw Data Dashboard** is the one to display the IR values on a gauge. To do that, just drag the node, and configure it as seen below. This will need to be done for any values that need to be displayed, whether as a graph, gauge, or simply a number, among other options. Playing around with the dashboard components to customize the look and feel of the UI will you to further understand how to use them.

![Alt Text](https://j.gifs.com/MQ9P25.gif)

You can import the final version of the flow from the file **Node-RED flow_Full.json**, which can be found in this repository.
